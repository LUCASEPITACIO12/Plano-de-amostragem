...
from calculos import Sistema, LinhaPlano, gerar_plano, calc_anexo14, pontos_rede, faixa_populacional, MESES
...
def aba_resumido(wb, sistemas: list[Sistema], ano: int):
    ...
    hdrs = [
        ...
        "Empresa Trat.", "Empresa Dist.", "Resp. Tratamento", "RT (Conselho/Nº)",
    ...
    escopo_label = {
        "rede":     "Somente Distribuição (rede)",
        "cap_trat": "Captação + Tratamento (produtor)",
    ...
    for i, s in enumerate(sistemas):
        ...
        row_data = [
            ...
            s.tratamento, s.manancial_efetivo,
            f"{s.empresa_responsavel}", f"{s.empresa_distribuicao}", f"{s.responsavel_tratamento}", f"{s.rt_nome} | {s.rt_conselho} {s.rt_registro}",
